...
1. Read repository instructions, the card, approved plan/specification, and commander-owned interface contract.
2. During an unapproved design gate, perform read-only reconnaissance only: map data flow, risks, and a proposed contract without editing business code.
3. After approval, modify only owned paths and preserve compatibility unless the card explicitly changes the contract.
